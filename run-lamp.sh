...
#Install PHPLogin
cd /var/www/html/ && /usr/local/bin/composer install 
...
# Run MariaDB
/usr/bin/mysqld_safe --timezone=${DATE_TIMEZONE} --skip-grant-tables &
sleep 5
/usr/bin/mysql -u root -e "CREATE DATABASE phpLogin"
/usr/bin/mysql -u root -e "CREATE USER 'phplg'@'localhost' IDENTIFIED BY 'password1234'"
/usr/bin/mysql -u root -e "GRANT ALL PRIVILEGES ON *.* TO 'phplg'@'localhost'"
